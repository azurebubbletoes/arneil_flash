package QuizApp.view.panels.progress.view
{
	...
	public class ProgressMediator extends Mediator implements IMediator
	{
		...
		public function ProgressMediator(viewComponent:CategoryPanel)
		{
		...
		override public function onRegister():void
		{
			...
			categoryPanel.addEventListener(CategoryPanel.ENABLE, enable, false, 0, true);
			categoryPanel.addEventListener(CategoryPanel.DISABLE, disable, false, 0, true);
			categoryPanel.addEventListener(CategoryPanel.PROGRESS, progress, false, 0, true);
			categoryPanel.addEventListener(CategoryPanel.NEXT, next, false, 0, true);
		...
		public function get categoryPanel():CategoryPanel
		{
			return viewComponent as CategoryPanel;
		}
		...
		private function handleChosen(e:Event):void
		{
			...
			if (b.label != "Select Category")
			{
				...
				(viewComponent as CategoryPanel).dispatchEvent(g);
		...
		{
			//trace("inner mediator sa enable")
			sendNotification(CategoryPanel.ENABLE);
		
		}
		
		private function disable(e:Event):void
		{
			sendNotification(CategoryPanel.DISABLE);
		
		}
		
		private function progress(e:GlobalEvent):void
		{
			//trace("marinelaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
			sendNotification(CategoryPanel.PROGRESS, e.body);
		
		}
		
		private function next(e:GlobalEvent):void
		{
			//trace("next! inner mediator.")
			sendNotification(CategoryPanel.NEXT, e.body);
